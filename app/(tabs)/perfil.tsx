...
export default function PerfilScreen() {
  ...
  const nome = profile?.nome ?? 'Explorador';
  const nivel = profile?.nivel ?? 1;
  const xp = profile?.xp_total ?? 0;
  const moedas = profile?.moedas ?? 0;
  const xpProximoNivel = nivel * 100;
  const progresso = Math.min((xp % xpProximoNivel) / xpProximoNivel, 1);
      ...
      >
        ...
        <Card variant="default" padding="md" className="mb-5">
          ...
          <View className="bg-slate-100 rounded-full h-3">
            <View className="bg-violet-500 h-3 rounded-full" style={{ width: `${progresso * 100}%` }} />
          </View>
        </Card>
